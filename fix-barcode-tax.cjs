const fs = require('fs');
let content = fs.readFileSync('src/components/printing/TaxInvoice.tsx', 'utf8');
...
if (!content.includes('<Barcode value={')) {
    content = content.replace(
        ...
        <div className="flex flex-col items-center justify-center py-2 mb-4">
          <Barcode value={invoiceNumber} width={1.8} height={50} fontSize={14} margin={0} displayValue={true} />
        </div>
...
fs.writeFileSync('src/components/printing/TaxInvoice.tsx', content);
