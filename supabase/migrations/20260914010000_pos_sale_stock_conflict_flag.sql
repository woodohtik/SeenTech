-- =============================================================================
--  Stock-deduction conflicts no longer fail the whole sale
--  (seen-offline-sync-architecture-task.md Phase 4)
--  ---------------------------------------------------------------------------
--  Phase 0's create_pos_sale deliberately rolled back the entire sale on ANY
--  failure, including insufficient stock -- correct for a purely-online
--  system with no legitimate way for two devices to oversell the same item
--  concurrently. Once Phase 3 lets two offline POS devices both sell the last
--  unit of something before either can sync, that same all-or-nothing
--  behavior becomes wrong: the customer already paid on both devices, and
--  neither payment can be undone by refusing to record the sale.
--
--  Fix: wrap each per-item stock movement in its own nested block (an
--  implicit plpgsql savepoint) so a failed deduction rolls back only that
--  movement, not the order/items/invoice/audit log already inserted. The
--  affected order_items row is flagged instead of silently dropped or
--  silently ignored -- someone has to reconcile it by hand later, but the
--  sale itself, and every other line in it, stands.
-- =============================================================================

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS stock_conflict BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS stock_conflict_reason TEXT;
