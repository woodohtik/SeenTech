...
-- (database-reviewer): create_pos_sale's per-line stock-application block
-- caught `EXCEPTION WHEN OTHERS`, silently relabeling ANY error (a
-- permissions problem, a query timeout, a genuine bug) as a "stock
-- conflict" on that order line and letting the sale complete anyway.
-- apply_stock_movement only ever raises the real insufficient-stock
-- condition with ERRCODE = '23514' (check_violation) -- every other
-- exception it can raise (missing operation_id: '22023', forbidden:
-- '42501') is a programming/authorization error that should fail the
-- whole sale loudly instead of being masked as a business-as-usual stock
-- conflict. Narrowed to that one SQLSTATE; everything else now
-- propagates and rolls back the transaction as it should.
--
-- Full CREATE OR REPLACE reproduced from
-- 20260917020000_zatca_pih_fix_digest_schema.sql with only the two
-- EXCEPTION clauses changed.
...
BEGIN
  ...
  -- extensions.digest -- see 20260917020000's header comment.
