...
/**
 * Persistent reminder of the post-onboarding "quick start" checklist. Shown on
 * every page (until fully complete) so leaving the one-time TourFinishModal
 * without finishing the steps doesn't lose the task entirely — dismissing it
 * only hides it for the current session; it comes back next login.
 */
export default function SetupChecklistBar({ tenantId, hasPermission }: SetupChecklistBarProps) {
  ...
  const dismissKey = tenantId ? `setup_checklist_dismissed_${tenantId}` : null;
  const [dismissed, setDismissed] = React.useState(
    () => !!dismissKey && sessionStorage.getItem(dismissKey) === 'true'
  );
  const [expanded, setExpanded] = React.useState(false);

  if (loading || isComplete || dismissed || !tenantId || totalCount === 0) return null;
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ y: -60, opacity: 0 }}
        className="fixed top-0 left-0 right-0 z-[150] bg-surface border-b border-border shadow-md"
        dir={isRtl ? 'rtl' : 'ltr'}
      >
        <div className="flex items-center gap-3 px-4 py-2.5">
          ...
          {dismissKey && (
            <button
              ...
              className="shrink-0 p-1.5 rounded-lg text-content-muted hover:text-content hover:bg-surface-muted transition-colors"
            >
              <XIcon size={14} />
            </button>
          )}
