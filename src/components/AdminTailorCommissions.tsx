...
export default function AdminTailorCommissions({ tenantId }: AdminTailorCommissionsProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12 bg-surface rounded-2xl shadow-sm border border-border h-64">
  ...
  return (
    <div className="bg-surface rounded-3xl shadow-sm border border-border overflow-hidden">
      <div className="p-6 border-b border-border flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-content">{t('settings_page.staff.commissions.title', 'عمولات الخياطين')}</h2>
...
function TailorCommissionRow({ 
  ...
  return (
    <div className="p-6 hover:bg-surface-muted/50 transition-colors flex flex-col md:flex-row md:items-center gap-6">
      <div className="flex-1">
        <h3 className="font-bold text-content text-lg">{tailor.name}</h3>
        <p className="text-sm text-content-muted mt-1" dir="ltr">{tailor.phone}</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 flex-1 max-w-2xl">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-4 pr-10 py-2.5 bg-surface border border-border rounded-xl outline-none transition-all focus:ring-2 focus:ring-brand/20 focus:border-brand font-bold text-content text-left"
            dir="ltr"
          />
        </div>
        ...
        <button
          ...
          className="px-6 py-2.5 bg-brand text-white font-bold rounded-xl hover:bg-brand/90 disabled:opacity-50 disabled:bg-surface-muted disabled:text-content-muted transition-all flex items-center justify-center gap-2 min-w-[120px]"
        >
          {isSaving ? (
