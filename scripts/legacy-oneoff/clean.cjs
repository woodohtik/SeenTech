...
// Find all occurrences of the bad duplicated block and replace it
content = content.replace(/\{formData\.category === "ready_made" && \(\s*<motion\.div\s*initial=\{\{ height: 0, opacity: 0 \}\}\s*animate=\{\{ height: "auto", opacity: 1 \}\}\s*exit=\{\{ height: 0, opacity: 0 \}\}\s*className="overflow-hidden"\s*>\s*\{formData\.category === "ready_made" && \(\s*<motion\.div\s*initial=\{\{ height: 0, opacity: 0, overflow: "hidden" \}\}\s*animate=\{\{ height: "auto", opacity: 1, transitionEnd: \{ overflow: "visible" \} \}\}\s*exit=\{\{ height: 0, opacity: 0, overflow: "hidden" \}\}\s*>/g, 
